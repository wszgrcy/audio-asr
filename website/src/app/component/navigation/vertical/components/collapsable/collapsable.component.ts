...
import { NavigationComponent } from '../../../vertical/vertical.component';
...
@Component({
  ...
  imports: [
    ...
    forwardRef(() => NavigationCollapsableItemComponent),
...
export class NavigationCollapsableItemComponent implements OnInit, OnDestroy {
  ...
  private _navigationComponent = inject(NavigationComponent);
  ...
  ngOnInit(): void {
    ...
    this._navigationComponent.onRefreshed
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe(() => {
        // Mark for check
    ...
    for (const child of children) {
      ...
      if (
        ...
        this._router.isActive(
          child.link,
          child.activateMatchOptions || SubsetMatchOptions,
        )
      ) {
        return true;
      }
